...
Dial.prototype.createDefs = function() {
    ...
    stop1.setAttribute('stop-color', '#d5137c');
    stop1.setAttribute('offset', '0%');
    linearGradient.appendChild(stop1);
    var stop2 = document.createElementNS("http://www.w3.org/2000/svg", "stop");
    stop2.setAttribute('stop-color', '#2da7df');
    stop2.setAttribute('offset', '100%');
    linearGradient.appendChild(stop2);

    var linearGradientBg = document.createElementNS("http://www.w3.org/2000/svg", "linearGradient");
    linearGradientBg.setAttribute('id', 'gradientBg');
    var stop1 = document.createElementNS("http://www.w3.org/2000/svg", "stop");
    stop1.setAttribute('stop-color', '#ffffff');
    stop1.setAttribute('offset', '0%');
    linearGradientBg.appendChild(stop1);
    var stop2 = document.createElementNS("http://www.w3.org/2000/svg", "stop");
    stop2.setAttribute('stop-color', '#ffffff');
    stop2.setAttribute('offset', '100%');
    linearGradientBg.appendChild(stop2);
    ...
    defs.appendChild(linearGradientBg);
...
Dial.prototype.createSliceBg = function() {
    var sliceBg = document.createElementNS("http://www.w3.org/2000/svg", "path");
    sliceBg.setAttribute('fill', 'none');
    sliceBg.setAttribute('stroke', 'url(#gradientBg)');
    sliceBg.setAttribute('stroke-width', this.strokeWidth);
    sliceBg.setAttribute('transform', 'translate(' + this.strokeWidth / 2 + ',' + this.strokeWidth / 2 + ')');
    sliceBg.setAttribute('class', 'animate-draw');
    this.svg.appendChild(sliceBg);
    this.sliceBg = sliceBg;
};
...
Dial.prototype.createText = function() {
    ...
    text.setAttribute('font-family', '"museo-slab", sans-serif');
...
Dial.prototype.setValue = function(value) {
    var circleValue = parseInt(value) + 100;
    var c = (circleValue / 200) * 360;
    if(c === 360)
      c = 359.99;
    var xy = this.size / 2 - this.strokeWidth / 2;
    var d = this.describeArc(xy, xy, xy, 180, 180 + c);
    var e = this.describeArc(xy, xy, xy, 180, 180 + 359.99);
    this.slice.setAttribute('d', d);
    this.sliceBg.setAttribute('d', e);
    var tspanSize = (this.size / 3.5) / 3;
    this.text.innerHTML = Math.floor(value) + '<tspan font-size="' + tspanSize + '" dy="' + -tspanSize * 1.2 + '"></tspan>';
};

$(document).ready(function($) {
  var containers = $(".chart");
